...
type Http3Stream interface {
	io.Reader
	SendHeader(status int, headers http.Header) (int, error) //send a generic frame type
...
func (s *RequestStream) SendHeader(status int, header http.Header) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Error when creating header frame: %s", err)
	}

	encodedFrame, err := headersFrame.Encode()
	if err != nil {
		return 0, fmt.Errorf("Error during varint encoding of the HEADERS frame: %s", err)
	}
...
func (s *RequestStream) Read(data []byte) (int, error) {
	...
	if s.remainingData == 0 {
		for !dataFrameRcv {
			...
			default:
				s.QuicStream.Close(http3.FrameUnexpected)
